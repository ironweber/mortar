...
(function( $ ){
	$.fn.mortar = function( options ) {
		var defaults = {
			'layout'		: 'hanging',
			'columnWidth'	: typeof options === 'undefined'  ? this.children(':first-child').outerWidth() : options.margins ? this.children(':first-child').outerWidth(true) : this.children(':first-child').outerWidth(),
			'margins'		: false,
			'mortarWidth'	: 20,
			'mortarHeight'	: 10			
		};
		...
		var containerHeight    = this.innerHeight(),
			containerWidth 	   = 0,
			firstKidHeight     = !options.margins ? this.children(':first-child').outerHeight() : this.children(':first-child').outerHeight(true),
		...
		if(firstKidHeight > containerHeight){
			containerKids.each(function(i){
				var $this 	   = $(this),
					$thisWidth = !options.margins ? $this.outerWidth() : $this.outerWidth(true),
					leftPos  = i === 0 ? 0 : ($thisWidth*(i+1)-$thisWidth)+(options.mortarWidth*i);
			...
			containerKids.each(function(i){
				var $this       = $(this),
					$thisHeight = !options.margins ? $this.outerHeight() : $this.outerHeight(),
					$thisWidth  = !options.margins ? $this.outerWidth() : $this.outerWidth(),
					leftPos, topPos;
				...
				if(i === 0){
					...
					topPos = topPos === 0 ? 0 : (contentHeight-$thisHeight)+(options.mortarHeight); // Calculate the top postions
					leftPos = numColumn === 1 ? 0 : (options.columnWidth*(numColumn-1))+(options.mortarWidth*(numColumn-1)); // Calculate the left postions
